...
void envoyerMessDGRAM (socket_t *sockEch, char *msg, char *adrDest, short portDest)  {
	...
	adr2struct(&addr, adrDest, portDest);

	CHECK(sendto(sockEch->fd, msg, strlen(msg)+1, SEND_FLAGS, (struct sockaddr *)&addr, sizeof(addr)), "--envoyerMessDGRAM--");

	sockEch->addrDst = addr;
}
/**
...
void recevoirMessDGRAM (socket_t *sockEch, char *msg, int msgSize)  {
	socklen_t lenAdr = sizeof(sockEch->addrDst);

    CHECK(recvfrom(sockEch->fd, msg, msgSize, 0, (struct sockaddr *)&sockEch->addrDst, &lenAdr), "--recevoirMessDGRAM--");
}
/*
*****************************************************************************************
 *	\noop		I M P L E M E N T A T I O N   DES   F O N C T I O N S
...
void envoyerMessSTREAM (const socket_t *sockEch, char *msg)  {
	CHECK(send(sockEch->fd, msg, strlen(msg)+1, SEND_FLAGS), "--envoyerMessSTREAM--");
}
/**
...
void recevoirMessSTREAM (const socket_t *sockEch, char *msg, int msgSize) {
	...
	CHECK(nbCar = recv(sockEch->fd, msg, msgSize, RECV_FLAGS), "--recevoirMessSTREAM--");
